...
let settle_step t =
  ...
  List.map t ~f:(fun brick ->
      let below = below brick in
      if is_oob below then brick
      else if intersects set below then brick
      else move_down brick)

(* XXX refactor to a sort of unfold *)
let settle t = fixpoint t ~f:settle_step ~equal:[%equal: t]

let%expect_test "settle" =
  parse sample |> settle |> [%sexp_of: t] |> print_s;
  [%expect
    {|
